...
export default async function handler(
  ...
  let details = await getDetails(object);
...
const getHtml = async () => {
  return new Promise(async (resolve, reject) => {
    fs.readFile("./lib/table.txt", "utf8", async (err, data) => {
...
const writeCsv = async (data: any) => {
  return new Promise(async (resolve, reject) => {
    //check if file exists if it does return it as a json object
    if (fs.existsSync("./lib/output.csv")) {
    ...
    for (let i = 0; i < data.length; i++) {
      if (data[i].url) {
        ...
        if (webpage) {
          ...
          await writeNewPropertyToFinalOutput(newObj);
...
const writeNewPropertyToFinalOutput = async (data: any) => {
  return new Promise(async (resolve, reject) => {
    //add the new property to the finalOutput.csv file
    const jsonArray = await csvtojson().fromFile("./lib/finalOutput.csv");
    jsonArray.push(data);
    const csv = new objectsToCsv(jsonArray);
    await csv.toDisk("./lib/finalOutput.csv", { allColumns: true });
    resolve(null);
  });
};
